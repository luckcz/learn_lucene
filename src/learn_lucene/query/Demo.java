...
public class Demo {
	 ...
	 */
	@Test
	public void testSearchDuanLuo() throws Exception{
		//1.创建IndexSearcher
		Directory d = FSDirectory.open(Paths.get(PATH)); //索引目录
		IndexReader reader = DirectoryReader.open(d);//索引读入器
		IndexSearcher indexSearcher = new IndexSearcher(reader);//索引搜索器
		//2.创建Query对象-把特定格式字符串解析得到
		String parseStr = "content:\"hello world\"";
		Analyzer analyzer = new SimpleAnalyzer();//用它对搜索条件分词
		String defaultField = "content";//默认搜索字段，如果搜索字符串中没有指定，就使用默认的
		QueryParser parse = new QueryParser(defaultField, analyzer);
		Query query = parse.parse(parseStr);
	...
	@Test
	public void testSearchQueryDuanLuo() throws Exception{
		//1.创建IndexSearcher
		Directory d = FSDirectory.open(Paths.get(PATH)); //索引目录
		IndexReader reader = DirectoryReader.open(d);//索引读入器
		IndexSearcher indexSearcher = new IndexSearcher(reader);//索引搜索器
		...
		TopDocs topDocs = indexSearcher.search(builder.build(), 10000);//查询top n个数据
		System.out.println("总长命中数："+topDocs.totalHits);
		ScoreDoc[] scoreDocs = topDocs.scoreDocs;//命中记录的数组
		for(ScoreDoc scoreDoc : scoreDocs){
			int docId = scoreDoc.doc;//获取文档编码
	...
	/**
	* @Title: testSearchDuanLuo 
	* @Description: 组合查询    与或非
	* @return void    返回类型
	* @author cz
	* @throws
	* @date 2019年2月15日
	 */
	@Test
	public void testSearchUnion() throws Exception{
		//1.创建IndexSearcher
		Directory d = FSDirectory.open(Paths.get(PATH)); //索引目录
		IndexReader reader = DirectoryReader.open(d);//索引读入器
		IndexSearcher indexSearcher = new IndexSearcher(reader);//索引搜索器
		//2.创建Query对象-把特定格式字符串解析得到
		//String parseStr = "content:hello content:world";//或
		//String parseStr = "+content:hello +content:world";//与
		String parseStr = "+content:hello -content:java";//有hello 无java
		Analyzer analyzer = new SimpleAnalyzer();//用它对搜索条件分词
		String defaultField = "content";//默认搜索字段，如果搜索字符串中没有指定，就使用默认的
		QueryParser parse = new QueryParser(defaultField, analyzer);
		Query query = parse.parse(parseStr);
	...
	/**
	* @Title: testSearchQueryDuanLuo 
	* @Description: query组合查询
	* @return void    返回类型
	* @author cz
	* @throws
	* @date 2019年2月15日
	 */
	@Test
	public void testSearchQueryUnion() throws Exception{
		//1.创建IndexSearcher
		Directory d = FSDirectory.open(Paths.get(PATH)); //索引目录
		IndexReader reader = DirectoryReader.open(d);//索引读入器
		IndexSearcher indexSearcher = new IndexSearcher(reader);//索引搜索器
		...
		builder.add(new TermQuery(new Term("content","hello")),Occur.MUST);
		builder.add(new TermQuery(new Term("content","java")),Occur.MUST_NOT);
		TopDocs topDocs = indexSearcher.search(builder.build(), 10000);//查询top n个数据
		System.out.println("总长命中数："+topDocs.totalHits);
		ScoreDoc[] scoreDocs = topDocs.scoreDocs;//命中记录的数组
		for(ScoreDoc scoreDoc : scoreDocs){
